...
<div class="border rounded shadow-xl hover:-translate-y-5 transition">
	{#if type == 'public'}
		...
		<div class="p-4">
			<h3 class="font-medium text-xl mb-2">{course.name}
	...
	{:else if type == 'admin'}
	<button on:click={() => goto(`/manager/moderationcourses/detail/${course?.courseId}`)}><img  alt="courseimg" src={course.coursePicture} class="w-full" /></button>
		
		<div class="p-4">
			<h3 class="font-medium text-xl mb-2">{course.courseName}
			</h3>
