<script lang="ts">
	...
	const AddExam = async() => {
		pageStatus.set("load")
		try{
			console.log(JSON.stringify({chapterId: chapter.id, lastExam: Exam}))
			const response = await addExam({chapterId: chapter.id, lastExam: Exam})
