...
<div class="flex">
    ...
    <div class="w-4/5">
        <div>
            ...
            {#if course?.tag == "Java"}
            <CodeEditor2 readonly={true} bind:lang={course.tag} bind:value={codeQuestion.testCaseJava}/>
            {:else if course?.tag == "C"}
            <CodeEditor2 readonly={true} bind:lang={course.tag} bind:value={codeQuestion.testCaseC}/>
            {:else if course?.tag == "C++"}
            <CodeEditor2 readonly={true} bind:lang={course.tag} bind:value={codeQuestion.testCaseCplus}/>
            {/if}
